...
SECTOR_BETA = {
    'tldr': '<b>核心叙事</b>：Intel Q1 2026 财报炸裂——EPS $0.29（共识 $0.01）、营收 $13.58B、Q2 指引 $13.8-14.8B 三杀超预期，<b class="up">INTC +23.64%</b> 创 2000 年以来历史新高。这不是单只 alpha，而是 <b>整个 CPU 设计板块 beta 重新被定价</b>：池里 INTC 在"CPU 处理器"标签（n=1）、ARM/QCOM 在"Fabless 设计"标签、AMD 在"AI 加速"标签，分散在三个 INDUSTRY_MAP 子行业，但本质是同一组 CPU/SoC 设计公司被市场集体重估，背后逻辑是 GB200 NVL72 单台 36 颗 Grace CPU + 72 颗 B200 GPU 的 BOM 结构，AI 算力栈从"GPU 独大"切换为"CPU + GPU + Networking"三足鼎立。<br><br><b>板块脉络</b>：当日 24 个子行业里 16 个 cap-w ≥ +1%，是普涨格局。<b class="up">CPU 处理器 +23.64%</b>（仅 INTC）、<b class="up">Fabless 设计 +12.37%</b>（n=29 全员涨，RMBS +14.37% / SYNA +10.49% / AMBA +9.10% 中小盘也跟涨）是最强 beta；<b class="up">封测 OSAT +7.17%</b>（AMKR/ASX/IMOS）+ <b class="up">晶圆代工 +4.99%</b>（TSM/UMC/GFS 等）是 AI 算力 capex 沿产业链下游传导；<b class="up">EMS 制造 +4.63%</b> CLS（AI ODM）领涨；<b class="up">光通信 +2.89%</b> MXL 单日 +76.12% 冠军 + AAOI +17.74% 引爆 800G/1.6T 互联估值。SOX <b class="up">+5.0%</b> 录得史上最长 18 连阳，NDX <b class="up">+1.9%</b> 创新高。<br><br><b>弱势板块</b>：仅 <b class="down">能源电池 -1.47%</b>（户用太阳能业绩前抛压）+ 消费电子 -0.87%（关税担忧），其余板块全部上涨。<b>后市看点</b>：5/6 ARM 财报（v9 royalty rate 上调验证）→ 5/28 NVDA 财报（DCAI 营收 + Q2 指引）→ 6 月 Computex 是 CPU beta 故事能否延续的三大关键节点。',
    'themes': [
        {
            'theme': '🔥 CPU 设计板块 beta：Intel 财报点燃 → AI 算力栈三足鼎立（INTC + ARM/QCOM + AMD 跨 3 个 INDUSTRY_MAP 标签集体重估）',
            'sectors': ['CPU处理器', 'Fabless设计', 'AI加速'],
            'sentiment': 'bull',
            'driver': 'INTC Q1 2026 财报全面超预期：EPS $0.29 vs 共识 $0.01（超 29 倍）、营收 $13.58B、Q2 指引 $13.8-14.8B，DCAI 数据中心营收同比加速。<b>关键是市场把这一信号外溢到整个"CPU/SoC 设计公司"集合</b>：池子里这些公司分布在 INDUSTRY_MAP 三个标签下——INTC 在"CPU 处理器"（n=1，板块即 INTC 本身 +23.64%）、ARM (+14.76%) / QCOM (+11.12%) 在"Fabless 设计"（板块 cap-w +12.37%）、AMD (+13.91%) 在"AI 加速"（板块 cap-w +4.01%，被 NVDA 的 +4.32% 大权重稀释了，但 AMD 自身涨 13.91%）。背后逻辑：GB200 NVL72 单台 BOM 配 36 颗 Grace CPU × $1000 + 72 颗 B200 GPU × $40000 + Spectrum-X 互联，CPU 从"AI 算力栈辅助"升级为"三足鼎立"。Fabless 板块全员同向（RMBS +14.37%、SYNA +10.49%、AMBA +9.10%、SIMO +8.07% 中小盘也跟涨）说明这是真 beta 不是单点 alpha。',
            'cross_sector': '<b>联动板块</b>：① <b>AI 服务器</b>（板块 cap-w +1.46%，被 NVDA 关联拖低，但 SMCI 单股 +5.4% 反映 GB200 ODM 订单可见度）② <b>封测 OSAT +7.17%</b> + <b>晶圆代工 +4.99%</b>（AMKR/TSM）—— CPU 设计公司订单 → 上游晶圆 + 下游封装同步受益 ③ <b>半导体设备 +3.72%</b>（KLAC/AMAT/LRCX）2nm 节点新一轮 capex 周期。这 4 条联动共同验证"AI 算力 capex 沿产业链多极扩张"主线。',
            'duration': '本季业绩窗口期是连续 catalyst（5/6 ARM v9 royalty rate / 5/28 NVDA DCAI 指引 / 6 月 Computex）；6-12 个月看 ASIC 化（Google TPU v7 / Meta MTIA / AWS Trainium 3）对通用 CPU/GPU 替代速度，可能从 H2 26 开始反转。',
        },
        {
            'theme': '🔥 半导体后段产业链 beta：AI 算力 capex 沿"代工→封测"上下游同向传导',
            'sectors': ['封测OSAT', '晶圆代工'],
            'sentiment': 'bull',
            'driver': '封测 OSAT 板块 cap-w <b>+7.17%</b>（n=3 全员涨：AMKR + ASX + IMOS）+ 晶圆代工板块 cap-w <b>+4.99%</b>（n=5 全员涨：TSM + UMC + GFS + TSEM + SKYT）双双跑赢大盘 +3.26%。这是真"上下游 beta"——同一笔 AI 算力 capex 从 fab（晶圆代工）流向 OSAT（先进封装），两端同时受益。<b>具体驱动</b>：① TSMC 2nm + CoWoS-L 满产，单 wafer 经济上 OSAT 后段封装价值占比从 7nm 时代的 ~10% 提升至 2nm 时代的 ~25%；② AMKR 美国 Arizona Vista 厂 $20 亿 CHIPS Act 投资进入产能爬坡，承接 TSMC AZ Fab 的后段封装需求；③ ASE (ASX) Apple A19/M5 SiP + AMD MI300/MI400 封装订单延续。<b>不是凑数</b>：封测 + 代工是真上下游业务关系，且当天 cap-w 涨幅都远超大盘平均，符合跨板块 beta 的硬标准。',
            'cross_sector': '<b>联动板块</b>：① <b>EMS 制造 +4.63%</b>（CLS / FLEX 等 AI ODM）是同一传导链下一段（封装好的芯片 → 服务器整机）② <b>半导体设备 +3.72%</b>（AMAT/LRCX/KLAC）是 capex 直接接收方 ③ <b>测试仪器</b>（KEYS/TER）封装后测试需求增长。完整传导：设备投资 → 代工产能 → 封测产能 → ODM 整机 → 算力上线。',
            'duration': '2026 年 TSMC 2nm 量产 + AMKR Arizona 厂投产是确定性 catalyst；中期看先进封装产能（CoWoS）扩张是否能跟上 AI 需求（目前供需缺口仍 30%+）。',
        },
        {
            'theme': '🔥 光通信板块 beta：MXL 引爆 800G/1.6T 互联估值重估',
            'sectors': ['光通信'],
            'sentiment': 'bull',
            'driver': '光通信板块 cap-w <b>+2.89%</b>（n=13），由 MaxLinear (MXL <b>+76.12%</b>，单日涨幅冠军 + 全池第一) 引爆——Q1 2026 营收 $137.2M（+43% YoY）+ FY26 光数据中心业务营收目标从 ~$125M 上调至 $160M（+28%），Keystone 800G/1.6T DSP 锁定 hyperscaler 2026H2 量产订单。板块内中小盘全员同向：AAOI <b>+17.74%</b>、CRDO <b>+5.12%</b>、FN <b>+4.46%</b>、LITE <b>+4.10%</b> 集体大涨；只有 COHR (-0.47%)、CIEN (+0.96%) 表现偏弱反映该子板块内的分化（Coherent 受 Spectrum-X 替代 InfiniBand 担忧压制）。<b>市场重新定价</b>：AI 服务器 BOM 中光模块 + 高速 cable 占比从"辅助"升至 ~15%（GB200 NVL72 内 NVLink cable 单台价值 $5 万），是仅次于 GPU 的第二大成本项。<b>注意特例 GLW (+3.77%)</b>：池里我们把 Corning 归在"连接器元件"子行业（因为 Corning 业务跨光纤/玻璃/陶瓷），但 4/24 GLW 的拉升驱动来自其 Optical Communications 业务（AI 数据中心光纤需求 +30% YoY），<b>实质属于光通信主题</b>，是 INDUSTRY_MAP 归类粗糙的特例。',
            'cross_sector': '<b>联动板块</b>：① <b>测试仪器</b>（VIAV / KEYS）800G/1.6T 光模块生产测试需求增长 ② <b>晶圆代工</b>（TSMC）光通信 DSP 都跑在 7nm/5nm 节点。<b>明确不联动</b>：连接器元件板块（cap-w 仅 +1.10%，跑输大盘）当天没有 beta，APH (-0.31%) / TEL (-1.41%) 龙头反而下跌——这跟之前误把"连接器"拉进光通信主题不同，本主题严格只写真动的板块。',
            'duration': '光模块 ASP 在 800G→1.6T 切换中提升 2.5 倍，Coherent / MXL / FN 等龙头 2026-2027 订单可见度高；中期看 silicon photonics 集成度是否颠覆传统可插拔模块路线。',
        },
        {
            'theme': '🟢 户用太阳能板块 beta：业绩前抛压 + IRA 政策不确定性',
            'sectors': ['能源电池'],
            'sentiment': 'bear',
            'driver': '能源电池板块 cap-w <b>-1.47%</b>（n=5），是当日 24 个子行业里唯一显著走弱的板块（其他下跌板块都在 -1% 内噪声范围）。ENPH 4/28 财报前抛压最重，市场担忧三个层面：① 特朗普政府对 IRA 法案 30% ITC 税收抵免态度未明，住宅太阳能行业生死线 ② 加州 NEM 3.0 改革后装机大幅下滑，是否触底待 Q1 数据验证 ③ 部分零部件依赖中国，关税升级影响成本结构。NVX/ELBM/ASTI/TOYO 等小盘储能厂商同步下跌反映整个板块的政策敏感度。<b>跟普涨格局形成对照</b>：当其他 23 个板块要么大涨要么小幅波动时，唯独这个板块独立走弱，反映"政策风险敞口"是 4/24 唯一被 punish 的特征。',
            'cross_sector': '<b>有限联动</b>：消费电子板块 cap-w -0.87% 也偏弱，部分小盘（VUZI / WLDS）受同样关税担忧拖累；但消费电子板块里关税敏感度更分散，不构成跟能源电池强联动。属于"政策风险板块"独立集群。',
            'duration': '4/28 ENPH 财报是关键短期 catalyst，若 EPS $0.43 共识达成 + Q2 指引平稳可触发短期反弹；中期看 IRA 法案修订进度（共和党控制国会下能否保留 ITC 税收抵免）。',
        },
    ],
}
