...
| 2026-06-04 | GH Actions FMP 延迟约 **72 分钟**（22:30 UTC cron → 23:42 UTC 6/4 数据 push 到 remote main），明显低于 6/3 的 100 min 天花板，回归到 5/19 教训 65-75 min 区间；单 routine 无并发 race；本次特殊情况是 remote auto-commit 先到（4993940），本地 claude/<random> 工作分支需 `git pull origin main` 拉取（5/22 教训的「双重 poll」中的 git-fetch 检查 remote 路径今日首次实战命中，本地文件 poll 路径未命中）。本日数据 313/313 hit、0 missing；NVDA / TSM / MRVL 三巨头逆势涨 + AVGO -12.59% 蒸发 $3,200 亿单股事件（24 月最大），CIEN -13.66% 财报后估值 derate 单股事件，整体属典型 tactical rotation 日；PSTG 6/4 close=$86.22 仍 stale（连续第 9 个交易日，5/27 教训跟踪中）。Monitor 第一次 30 min timeout、重新 arm + bg Bash 60s poll 双路并行，bg Bash 在 6/5 00:42 UTC（23:42 UTC）先命中（remote auto-commit），主 routine 拉取后正常进入 narrative 写作 + gen.py + 准备 commit + PR | 根因：6/3 的 100 min 极端延迟是单日异常，6/4 回归 65-75 min 常态区间；本地 file poll 路径今日未命中（因为本地未跑 GH Actions、文件先到 remote），只能靠「double-poll」中的 git fetch 路径命中 | **判断准则保持 6/3**：(1) 默认等待 70-100 min，不要在 70 min 前 panic；(2) Monitor `until ls confirmed_*.json \|\| (git fetch && git log origin/main \| grep "FMP daily fetch")` 双路 poll 是稳定方案；(3) 当 remote auto-commit 先到时，主 routine 直接 `git pull origin main` 即可获取 confirmed + macros 文件 + earnings_history + _meta（一次 fast-forward），不需要 cherry-pick 单文件；(4) 本日 routine 全程顺利：cp 6/3 模板 → Write 单次 ~28KB narrative JSON（5/8 教训方案）→ gen.py 无 sub-threshold 警告 → 8 KEY_STOCKS（AVGO/CIEN/MU/NVDA/MRVL/TSM/AAOI/CSCO）+ 5 themes 全数据真实验证 → commit + PR 路径 |
