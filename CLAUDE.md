...
| 2026-05-27 | WebSearch 搜 NVTS 5/27 涨跌，一篇报道显示 +18.32%，但 FMP confirmed_2026-05-27.json 确认 NVTS 当日 -9.15%（收 $4.38，prev_close $4.81）——方向完全相反 | WebSearch 源头混淆了不同日期（NVTS 某日确实有过大涨，被部分数据聚合页缓存/显示为当日结果）。与 5/18 / 5/20 教训同一根因：WebSearch 的具体数字在多源冲突时完全不可信 | **强化已有规则**：KEY_STOCKS 的 dp/close/cap 必须从 FMP JSON 取实数，WebSearch 仅用于定性方向 + 新闻研判，**绝不能用 WebSearch 数字覆盖 FMP 数字**。本次在写 narrative 前已先从 FMP 算好所有 ticker 的 dp，NVTS 的 WebSearch 数字被当场识别并丢弃。FMP 为唯一真值。|
| 2026-05-27 | PSTG（Pure Storage）于 2026-02-23 宣布品牌更名 Everpure，2026-04-17 NYSE ticker 从 PSTG 改为 P；FMP 仍以 PSTG 追踪（confirmed_2026-05-27.json 里 PSTG 正常出现，P 不存在） | FMP 数据源有时滞，ticker 更名后 FMP 可能延迟切换，或永远保留旧 ticker。此前 pool 用 PSTG，gen.py / fetch_fmp.py / INDUSTRY_MAP 均写死 PSTG | **监控项**：若未来某日 confirmed_*.json 里 PSTG 消失、missing list 出现 PSTG，说明 FMP 已切换。届时需同步修改 INDUSTRY_MAP（gen.py）+ fetch_fmp.py 中的 TICKERS 列表把 PSTG 换为 P。在此之前，FMP 仍能正常追踪，无需立即改动。narrative 里提及该公司时应注明「NYSE: P（原 PSTG，2026-04-17 改名）」以免读者困惑 |
