...
| 2026-06-02 | 主 routine 等待 FMP 约 90 分钟无数据（polling loop 全部超时），判断 GH Actions 未触发，改用 WebSearch 写 narrative_2026-06-02.json（含大量错误数字：NVDA 写 +2.34% 实际 -0.69%，DELL 写 +1.85% 实际 -6.56%，漏掉 COHR/LITE/GLW/STM 大涨板块）；session context 压缩重启后，git fetch 发现 GH Actions 实际已运行（hit 313/313，推测延迟约 80-90 min），remote main 已有 confirmed_2026-06-02.json 和 GH Actions 自动生成的 HTML | 根因一：GH Actions runner 排队延迟超过 routine Monitor poll 总时长（90 min），主 routine 放弃等待提前进入 WebSearch fallback 路径。根因二：session context 在写完 narrative 但未 commit 前被压缩重启，数据已写入磁盘但本地未 git pull，直到下一轮才发现 remote 已有数据。根因三：WebSearch 多源数据冲突（NVDA/DELL 方向完全相反），在无 FMP 比对的情况下很难识别 | **解法**：session 重启后先做 `git fetch && git log origin/main` 检查是否有新的 auto-commit；如果有，pull 并用真实数据重写 narrative（本次已执行，最终 narrative 用 FMP 真值）。**判断准则升级**：GH Actions 延迟已漂移到 80-90 min 区间（5/11=57/5/12=64/5/18=64/5/19=68/5/20=75/5/21=66/5/22=80/6/2=~85 min 估算），不能在 90 min 前下「GH Actions 未触发」的结论——应等到 100 min 或 git fetch 发现 remote 无新 commit 才切 fallback。**PSTG stale 持续**：confirmed_2026-06-02.json 中 PSTG close=$86.22 与 6/1 完全相同（FMP 仍返回 stale 数据，5/28 教训已记录，持续中）|
