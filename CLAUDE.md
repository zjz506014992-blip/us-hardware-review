...
| 2026-05-20 | GH Actions 5/20 auto-fetch 延迟至 23:46 UTC（cron 22:30 后 **76 分钟**），超过历史最高纪录 5/19 的 68 分钟，创新高。routine 用 background until+60s poll + Monitor 双重兜底检测，期间 context 跨越 session 压缩边界后恢复 | 连续 5 个交易日异常延迟（5/11=57min、5/12=64min、5/18=64min、5/19=68min、5/20=76min），GH Actions runner 排队峰值持续向 80 min 漂移。推测原因：北美晚高峰 + GH Actions 免费/付费 runner 混队 + 22:30 UTC 属于全球多 repo cron 高峰。5/20 还有 session 中断（context 超限触发 compaction）——routine 在 narrative JSON 写完后被迫中断，下个 session 续跑 gen.py + commit | **判断准则升级**（替代 5/19 教训）：(1) 22:30 cron 后默认等待 **70-80 min**（之前 65-75 min），不要在 70 min 时 panic；(2) background until+60s poll 是最可靠的兜底（不依赖 Monitor timeout）；(3) 如果 context 被 compaction 中断，下次 session 直接 `ls` 检查文件是否已存在，从 gen.py 步骤续跑即可（narrative JSON 幂等，已写完就不重写）；(4) narrative JSON 写完立即**验证 JSON 语法**（`python3 -c "import json; json.load(open('narrative_{DATE}.json'))"`）防止 session 断后无法验证。本次 76 min 等待期间完成：5/27 earnings_briefs 4 条 + 全量 WebSearch 预研 + narrative 完整草稿 |
